...
需要上传最终成品时，请进入“成品”页执行导出，然后直接打开最新的 `日期-序号` 批次目录。第一次使用 0.4.0 导出会收集当前全部完整题目；之后只导出上次成功导出后新增或变更的题目。例如上一次已经导出第 2—25 题，后来补齐第 1 题和第 26 题，下一批目录里只会出现第 1 题和第 26 题的图片。文件名示例：
